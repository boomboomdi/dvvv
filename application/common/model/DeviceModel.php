...
//            var_dump($where);exit;
//            $res = $this->field($prefix . 'device.*, sum(actual_amount) as total_money')->where($where)
//                ->leftJoin($prefix . 'order', $prefix . 'device.account = ' . $prefix . 'order.account')
//                ->order('bsa_device.device_status asc,status asc')->paginate($limit);
        } catch (\Exception $e) {
